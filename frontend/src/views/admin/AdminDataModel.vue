    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑模型' : '新增模型'" width="550px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="技术栈">
          <el-input v-model="form.techStack" placeholder="如：MySQL, Redis" />
        </el-form-item>
...
import {
  getDataModels, addDataModel, updateDataModel, deleteDataModel, batchDeleteDataModels
} from '../../api'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getDataModels({ page: page.value, size: pageSize, keyword: keyword.value || undefined })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await updateDataModel(editId.value, { ...form })
      ElMessage.success('编辑成功')
    } else {
      await addDataModel({ ...form })
...
const handleBatchDelete = async () => {
  try {
    ...
    await batchDeleteDataModels(selectedIds.value)
